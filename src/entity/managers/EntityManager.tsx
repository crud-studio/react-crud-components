...
const EntityManager: FunctionComponent<IProps> = ({
    ...
    const getEntity = useCallback((entityName?: string): Entity<any> => {
        return entityMap[entityName || ""];
    }, [entityMap]);

    const getEntityEnum = useCallback((enumName?: string): EnumInfoMap<any> => {
    ...
    const getEnumSelectOptions = useCallback((enumName?: string): SelectOption[] => {
        if (!enumName) {
            return [];
        }
    ...
    const getEnumValueSelectOption = useCallback((enumName: string, value: string): SelectOption | null => {
        const enumInfoMap = enumMap[enumName];
        if (!enumInfoMap) {
            return null;
        }
    ...
    const getEnumValuesSelectOptions = useCallback((enumName: string, values: string[]): SelectOption[] => {
        const enumInfoMap = enumMap[enumName];
        if (!enumInfoMap) {
            return [];
        }
